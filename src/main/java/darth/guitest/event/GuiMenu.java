...
public class GuiMenu {
    ...
    public Inventory getMenu(String configpath, Player p) {
        ...
        if (plugin.getConfig().getBoolean(configpath+".enablealias")) {
            ...
            inventory = Bukkit.createInventory(null, plugin.getConfig().getInt(configpath + ".size"), PlaceholderAPI.setPlaceholders(p, configpath.replaceAll("-", " ")));
        }
        try {
                ...
                for (String string : test) {
                    ...
                    if (plugin.getConfig().getString(configpath+".contents."+string+".lore") != null) {
                        ...
                        for (String line : rawlore) {
                            lore.set(Integer.parseInt(line)-1,ChatColor.translateAlternateColorCodes('$', PlaceholderAPI.setPlaceholders(p, plugin.getConfig().getString(configpath+".contents."+string+".lore."+line))));
                        }
                        meta.setLore(lore);
                    }
